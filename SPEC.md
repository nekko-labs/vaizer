...
The installable skills in the catalog live in this repo, under `plugins/`, and the repo doubles as a Claude Code plugin marketplace. Adding it once (`/plugin marketplace add nekko-labs/vaizer`) and installing `vaizer@vaizer` brings in every Nekko-official skill and keeps them updated. *Why:* a catalog that shows you how a skill runs and then hands you an install command should own the thing it hands you. One repo means the site entry, the workflow graph, and the skill itself move together and cannot drift; the earlier split (a separate `nekko-dojo-skills` repo, retired 2026-08-02) carried the old Dojo name and kept the catalog one repo away from its own contents. `[shipped]`

- This repo is a marketplace: `.claude-plugin/marketplace.json` + a single `plugins/vaizer/` plugin holding every skill under `skills/<skill>/`, installed in one step with `/plugin install vaizer@vaizer` `[shipped]`
- **One bundled plugin, not one plugin per skill** (changed 2026-08-28): Claude Code namespaces plugin skills as `plugin-name:skill-name`, so a plugin per skill invoked as `/codereview-spec:codereview-spec`, repeating itself. Naming the plugin after the marketplace gives `/vaizer:codereview-spec`, `/vaizer:nyaa`, `/vaizer:cr`, `/vaizer:domain-finder`, `/vaizer:resume-checker`. The trade is that installing takes all skills at once rather than one at a time, and every session carries all their descriptions (about 740 always-on tokens rather than about 160 for one) `[shipped]`
...
- **The PR description is the first yardstick** (added 2026-08-28): before the diff, Spectral reads the PR description and takes three things from it, the goal (one sentence: what this change should accomplish), the product context and value (who it is for and what improves for them, the one thing a diff can never show), and each specific claim it makes. Those claims are then traced through the diff to the place a user reaches them: a stated goal the code does not deliver, or a claim the diff does not contain, is blocking; substantial work the description never mentions is unexplained scope; a description that says what changed but never why it matters gets asked for the value. The verdict header carries a `Goal:` line reading met / partially met / not met `[shipped]`
- **A PR with no description is a finding, and gets one drafted** (added 2026-08-28): an empty body, an unfilled template, a one-word body, or a bare issue link all count as no description. Blocking when the diff changes behavior, a feature, a route, an interface, or a data shape, informational for refactor / build / test-only work, because without a stated goal nobody can say whether the change did its job. nyaa asks for one and drafts it from the diff as a copy-pasteable block (what changed, why and for whom, what to look at, how to verify), offered in the verdict rather than posted to the PR. It never grades a diff against a description it wrote itself, since code passes its own summary by construction, and the goal line stays `no description given`. On a working diff with no PR the goal comes from the branch name and commit messages and is labelled as such `[shipped]`
- **A change that conflicts with the spec is filed as a stale spec, not a rogue diff** (added 2026-08-28): when the description says the product now does X and the spec still says Y, the usual cause is a spec nobody updated, so the finding is reported against `[SPEC.md § section]` with the sentence that section should now carry. It is blocking either way, since a spec describing the old behavior is wrong rather than merely incomplete. The exception is a conflict with a stated non-goal, success criterion, or scope boundary: those are decisions, so the line is quoted and the author is asked whether the boundary moved or the change did. When the description and the spec disagree and the code matches neither, nyaa says so instead of picking a winner `[shipped]`
- **Spec conformance is the second lens** (Spectral 👻, added 2026-08-28): the spec is read before the diff, then the change is checked both ways. Code against stated intent (scope, vocabulary, nothing marked shipped while half-built), and spec against the change (a feature, behavior, route, interface, or data-shape change with no spec update is blocking; refactor / build / test-only is informational) `[shipped]`
...
- Reads the PR description alongside the spec (added 2026-08-28), checks the code actually delivers the goal it stated, treats a conflict with a spec feature as a stale spec, and reports a missing description as a finding with a drafted replacement `[shipped]`
