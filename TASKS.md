...
### Shipped
- [x] nyaa v1.1.0: fifth reviewer lens **Tama 📜 (spec conformance & product intent)**, now the skill's primary value. New `references/tama.md` checklist, a "load the spec" step before the diff is read (so the spec is the yardstick, unprimed by the implementation), two-way drift checking (code vs spec and spec vs code, with an unrecorded behavior change blocking), review of the spec itself (missing feature definitions, test expectations, folder structure, conventions, data model, non-goals, stale text) reported as `[SPEC.md]` findings, plus a `Spec:` line and a `Spec gaps` section in the verdict. Mirrored into `commands/cr.md`; description/tags/version synced across `SKILL.md`, `plugin.json`, `marketplace.json`, `catalog.json`, `src/data/skills.ts`; site workflow graph gained a "Read the spec" context node and a "Spec cat" agent node. SPEC.md previously had no nyaa section at all, so one was written, added+done 2026-08-28 · [Skills, nyaa](SPEC.md)
